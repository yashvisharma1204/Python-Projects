...
def peer():
    action = input("Do you want to send or receive a file? (send/receive): ").strip().lower()
    
    if action not in ['send', 'receive']:
        print("[!] Invalid action. Please choose 'send' or 'receive'.")
        return

    host = input("Enter the host IP address (default 127.0.0.1): ").strip() or '127.0.0.1'
    port = input("Enter the port number (default 5001): ").strip() or '5001'
    ...
    if action == 'receive':
        target_directory = input("Enter the target directory to save the file (default .): ").strip() or '.'
